...
public class Round356_div1_D_v2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int bcdCity = 0 ; bcdCity < n ; bcdCity++) {
            ...
            solution = Math.max(solution, pCatch(bcdCity, 0, n-1, pLimak, 1));
        }
    ...
    private static double pCatch(int bcdCity, int minDist, int maxDist, double[] pLimak, int movesLeft) {
        double prob = 0.0;
        // consider each group of cities with same distance from bcdCity
        for (int dist = minDist ; dist <= maxDist ; dist++) {
            Collection<Integer> distCities = citiesByDist[bcdCity].getOrDefault(dist, Collections.EMPTY_LIST);
            if (distCities.isEmpty()) {
                continue;
            }

            double pGroup = distCities.stream()
            ...
            double pCatchNow = distCities.stream()
                    .mapToDouble(c -> pLimak[c] / pGroup)
                    .max().orElse(0);
            // ... or let him make a move and then try to catch him
            double pCatchNext = 0;
            if (movesLeft > 0) {
                double[] pLimakNext = pLimakNext(pLimak, distCities);
                for (int distFromBcdCity = 0 ; distFromBcdCity < n ; distFromBcdCity++) {
                    Collection<Integer> nextBcdCities = citiesByDist[bcdCity].getOrDefault(distFromBcdCity, Collections.EMPTY_LIST);
                    for (int bcdCityNext : nextBcdCities) {
                        double pCatchNextCandidate = pCatch(bcdCityNext,
                                Math.max(0,   dist - 1 - distFromBcdCity),
                                Math.min(n-1, dist + 1 + distFromBcdCity),
                                pLimakNext,
                                movesLeft - 1);
                        pCatchNext = Math.max(pCatchNext, pCatchNextCandidate);
                    }
                }
            }

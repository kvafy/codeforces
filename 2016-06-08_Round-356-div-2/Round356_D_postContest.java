...
public class Round356_D_postContest {
    ...
    private static Pair recurse(long remaining) {
        ...
        if (remaining <= 0) {
            ...
            if (result == null) {
                ...
                long nextBlock = (cbrt - 1) * (cbrt - 1) * (cbrt - 1);

                Pair xp = recurse(remaining - block);
                Pair yp = recurse(block - 1 - nextBlock);
                if (xp.blocks > yp.blocks || xp.blocks == yp.blocks && xp.volume + block > yp.volume + nextBlock) {
                    result = new Pair(xp.blocks + 1, xp.volume + block);
                } else {
                    result = new Pair(yp.blocks + 1, yp.volume + nextBlock);
                }
                cache.put(remaining, result);
            }
        }

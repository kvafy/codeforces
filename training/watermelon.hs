...
-- the watermelon in such a way that each of the two parts weighs even number
-- of kilos, at the same time it is not obligatory that the parts are equal.
...
-- which is the weight of the watermelon bought by the boys.
...
main = do
  line <- getLine
  let w = read line :: Integer
  let answer = if w >= 4 && w `mod` 2 == 0
                 then "YES" else "NO"
  putStrLn answer
